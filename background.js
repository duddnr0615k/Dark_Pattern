	...
	function setBackGroundColor(){
		...
		for(const roles of document.querySelectorAll('div,ul')){
			...
			// 숨겨져 있는 항목 체크 (에듀윌,밀리의서재)
			if (roles.style.display == 'none') {	
				if((roles.innerHTML.includes('checkbox') && roles.textContent.includes('문자(SMS)')) || (roles.innerHTML.includes('checkbox') && roles.textContent.includes('제 3자 제공'))){
